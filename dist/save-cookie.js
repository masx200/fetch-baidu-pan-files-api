...
const a = o.join(i.homedir(), "baidupan", "./userdata"),
    t = o.resolve(a, "./bdstoken.txt"),
    l = o.resolve(a, "./cookies.json");

console.log(r.argv.slice(2));

const c = n(r.argv.slice(2)),
    m = c.cookie,
    p = c.bdstoken;
...
r.on("unhandledRejection", (o) => {
    ...
    (async () => {
        ...
        if (
            (await e.ensureDir(o.dirname(l)),
            await (async (o) => {
                console.log(o);
                const r = s.parse(o, {});
                console.log(r),
                    await e.writeJSON(l, r, {
                        spaces: 4,
                    });
            })(m),
        ...
        await e.ensureDir(o.dirname(t)),
            await e.writeFile(t, p),
